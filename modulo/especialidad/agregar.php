...
$query = "SELECT MAX(id_especialidad) AS max_id FROM especialidad";
...
if (isset($_POST['btnregistrar'])) {
    $camposRequeridos = ['txtid', 'txtespec'];
    if (validarCampos($camposRequeridos)) {
        ...
        $queryAdd = mysqli_query($conn, "INSERT INTO especialidad(id_especialidad, especialidad) VALUES('$idespecialidad', '$especialidad')");

        if (!$queryAdd) {
            echo "Error con el registro: " . mysqli_error($conn);
        } else {
            echo "<script>window.location= '../../mant_especialidad.php?pag=1' </script>";
        }
    } else {
...
<head>
 ...
 <script>
        ...
        function validarFormulario() {
            ...
            if (idespecialidad.trim() === '' || especialidad.trim() === '' ) {
...
<body>
	...
	<fieldset style=" height:1000px;">
        <form class="contenedor_popup" method="POST" onsubmit="return validarFormulario();">
                ...
                <fieldset class="caja">
                    <legend class="cajalegend">══ Nueva especialidad ══</legend>
                    <p style="margin:0;">
