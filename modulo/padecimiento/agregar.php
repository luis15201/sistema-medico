...
$query = "SELECT MAX(id_padecimiento) AS max_id FROM padecimientos_comunes";
...
if (isset($_POST['btnregistrar'])) {
    $camposRequeridos = ['txtid', 'txtnombre', 'txtdescripcion'];
    ...
    if (validarCampos($camposRequeridos)) {
        ...
        // Insertar datos en la tabla padecimientos
        $queryAdd = mysqli_query($conn, "INSERT INTO padecimientos_comunes( nombre_padecimiento, descripcion) VALUES( '$nombre', '$descripcion')");

        if (!$queryAdd) {
            echo "Error con el registro: " . mysqli_error($conn);
        } else {
            echo "<script>window.location= '../../mant_padecimientos_comunes.php?pag=1' </script>";
        }
    } else {
...
<body>
	...
	<fieldset style=" height:900px;">
        <form class="contenedor_popup" method="POST" onsubmit="return validarFormulario();">
                ...
                <legend>Registrar Padecimientos | Condición médica previa</legend>
                <fieldset class="caja">
                    <legend class="cajalegend">══ Nuev@ Padecimientos | Condición médica previa ══</legend>
                    <p style="margin:0;">
                ...
                <div class="botones-container">
                    ...
                    <a class="boton" href="../../mant_padecimientos_comunes.php?pag=<?php echo $pagina; ?>">
