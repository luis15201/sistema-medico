...
$pagina = $_GET['pag'];
$coddni = $_GET['id_medico'];
//INSERT INTO `medicos`(`id_medico`, `cedula`, `exequatur`, `nombre`, `apellido`, `id_especialidad`)

$querybuscar = mysqli_query($conn, "SELECT * FROM medicos WHERE id_medico =$coddni");

while ($mostrar = mysqli_fetch_array($querybuscar)) {
    $idMedico = $mostrar['id_medico'];
    $cedula = $mostrar['cedula'];
    $exequatur = $mostrar['exequatur'];
    $nombre = $mostrar['nombre'];
    $apellido = $mostrar['apellido'];
    $idEspecialidad = $mostrar['id_especialidad'];
}

$querybuscar2 = mysqli_query($conn, "SELECT * FROM especialidad WHERE id_especialidad = $idEspecialidad");
while ($mostrar = mysqli_fetch_array($querybuscar2)) {
    $idespec = $mostrar['id_especialidad'];
    ...
    <div class="container">
        <fieldset style=" height:1200px;">
            <form class="contenedor_popup" method="POST" onsubmit="return validarFormulario();">
                <legend>Editando Médico 👩‍⚕️🧑‍⚕️</legend>
                <fieldset class="caja">
                    <legend class="cajalegend">══ Modificando Médico | Doctor  🩺 ══</legend>
                    <p style="margin:0;">
                    ...
                    <p>
                        ...
                        <select name="selectespecialidad" id="selectespecialidad">
                            ...
                            while ($row = $result->fetch_assoc()) {
                                echo '<option ($especialidad == "'.$row["especialidad"].'") ? "selected" : value="' . $row["id_especialidad"] . '">' . $row["especialidad"] . '</option>';
                            }
                ...
                <div class="botones-container">
                    <button class="btn btn-primary boton" type="submit" name="btnmodificar" value="Modificar" onClick="javascript: return confirm('¿Deseas Modificar el registro del doctor?');"> <i class="material-icons" style="font-size:21px;color:white;text-shadow:2px 2px 4px #000000;">edit</i> modificar</button>


                    <?php echo "<a class='btn btn-primary boton' href=\"../../mant_medico.php?pag=$pagina\"><i class='material-icons' style='font-size:21px;text-shadow:2px 2px 4px #000000;vertical-align: text-bottom;'  >close</i> Cancelar</a>"; ?>
...
if (isset($_POST['btnmodificar'])) {
    ...
    $querymodificar = mysqli_query($conn, "UPDATE medicos SET id_medico='$idMedico',cedula='$cedula', exequatur='$exequatur', nombre='$nombre', apellido='$apellido',id_especialidad='$idEspecialidad' WHERE id_medico= $idMedico ");
    echo "<script>window.location= '../../mant_medico.php?pag=$pagina' </script>";
}
?>